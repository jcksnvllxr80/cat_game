...
export class Level4Scene extends Phaser.Scene {
  ...
  create() {
    ...
    setupThreeLevel(this, {
      ...
      worldWidth,
      worldHeight,
      platformKey: 'canyon_platform',
      groundKey: 'canyon_ground',
      pitGaps: [
        { start: 1400, end: 1520 },
        { start: 2800, end: 2920 },
        { start: 4200, end: 4320 },
  ...
  update() {
    ...
    if (isCleo) {
      ...
      if (touchingWall && this.cursors.up.isDown) {
        ...
        if (!this.player.anims.isPlaying || this.player.anims.currentAnim?.key !== 'cleo_climb') {
          this.player.play('cleo_climb');
        }
      } else {
  ...
  openPause() {
    this.scene.pause();
    this.scene.launch('PauseScene', { callerScene: 'Level4Scene' });
  }
